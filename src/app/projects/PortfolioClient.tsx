  ...
  return (
    <div className="bg-[#030712] min-h-screen text-white overflow-hidden" ref={containerRef}>
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[10%] left-[10%] w-[40%] h-[40%] rounded-full bg-[#2563EB]/10 blur-[150px]" />
        <div className="absolute bottom-[10%] right-[10%] w-[40%] h-[40%] rounded-full bg-[#00E5FF]/10 blur-[150px]" />
      </div>
      ...
      <section className="relative pt-40 pb-24 px-6 z-10 flex flex-col items-center text-center">
        <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-5xl mx-auto relative">
          <motion.div variants={fadeInUp} className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass border border-[#00E5FF]/30 mb-8 text-sm font-medium text-[#00E5FF]">
          ...
          <motion.h1 variants={fadeInUp} className="text-5xl md:text-7xl font-space-grotesk font-bold tracking-tighter mb-8 bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-gray-500 leading-tight">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00E5FF] to-[#8B5CF6]">Intelligent Solutions</span>
          </motion.h1>
          ...
          <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <Link href="#projects" className="w-full sm:w-auto px-10 py-5 rounded-xl bg-gradient-to-r from-[#00E5FF] to-[#2563EB] text-white font-bold text-lg hover:shadow-[0_0_40px_-10px_rgba(0,229,255,0.5)] transition-all flex items-center justify-center gap-2">
      ...
      <section id="projects" className="py-24 px-6 relative z-10 border-t border-white/10 bg-white/5">
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all border ${
                  activeCategory === cat 
                    ? "bg-[#00E5FF]/20 border-[#00E5FF] text-[#00E5FF] shadow-[0_0_15px_-3px_rgba(0,229,255,0.4)]" 
            ...
            <AnimatePresence>
              {filteredProjects.map((project) => (
                <motion.div
                  ...
                  className="glass rounded-3xl border border-white/10 overflow-hidden group hover:border-[#00E5FF]/50 transition-colors flex flex-col"
                >
                  <div className="h-48 bg-[#0a0f1c] relative overflow-hidden flex items-center justify-center border-b border-white/5 group-hover:bg-[#00E5FF]/5 transition-colors">
                    <div className="absolute inset-0 bg-gradient-to-br from-[#00E5FF]/10 to-transparent mix-blend-overlay" />
                    <div className="w-20 h-20 rounded-2xl glass border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-500 shadow-xl text-[#00E5FF]">
                      {project.icon}
                    </div>
                  </div>
                  <div className="p-8 flex-1 flex flex-col">
                    <div className="text-xs font-bold text-[#8B5CF6] uppercase tracking-wider mb-2">{project.category}</div>
                    <h3 className="text-2xl font-bold font-space-grotesk mb-3">{project.title}</h3>
                    <p className="text-gray-400 mb-6 flex-1 text-sm leading-relaxed">{project.desc}</p>
                    ...
                    <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                      <span className="text-xs text-gray-500 font-medium">Client: {project.client}</span>
                      <button className="text-[#00E5FF] hover:text-white transition-colors text-sm font-bold flex items-center gap-1">
      ...
      <section className="py-24 px-6 relative z-10">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-400 max-w-2xl mx-auto">Deep dives into how we solve complex enterprise challenges.</p>
          </div>

          <div className="space-y-12">
            {[
              { title: "Global Logistics AI Optimization", prob: "High fuel costs and inefficient routing.", sol: "Custom AI predictive routing model.", res: "30% reduction in fuel costs, 40% faster delivery times.", color: "from-[#00E5FF] to-[#2563EB]" },
              { title: "Healthcare Data Automation", prob: "Manual entry causing compliance risks.", sol: "OCR & NLP automated document processing.", res: "99.9% accuracy rate, 5,000+ hours saved monthly.", color: "from-[#8B5CF6] to-[#D946EF]" }
            ].map((study, i) => (
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="glass rounded-3xl border border-white/10 overflow-hidden flex flex-col md:flex-row">
                <div className={`w-full md:w-2/5 bg-gradient-to-br ${study.color} p-12 flex flex-col justify-center relative overflow-hidden`}>
                  ...
                  <button className="relative z-10 px-6 py-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl font-semibold backdrop-blur-md transition-colors w-fit flex items-center gap-2">
                    Read Full Case Study <ArrowRight size={16} />
                  </button>
                </div>
                <div className="w-full md:w-3/5 p-10 grid grid-cols-1 sm:grid-cols-2 gap-8">
                  <div>
                    <h4 className="text-[#00E5FF] font-bold mb-2 flex items-center gap-2"><Target size={18} /> The Problem</h4>
                    <p className="text-gray-400 text-sm">{study.prob}</p>
                  </div>
                  <div>
                    <h4 className="text-[#8B5CF6] font-bold mb-2 flex items-center gap-2"><Lightbulb size={18} /> The Solution</h4>
                    <p className="text-gray-400 text-sm">{study.sol}</p>
                  </div>
                  <div className="sm:col-span-2 pt-6 border-t border-white/5">
                    <h4 className="text-[#2563EB] font-bold mb-2 flex items-center gap-2"><Activity size={18} /> Business Results</h4>
                    <p className="text-white font-medium text-lg">{study.res}</p>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section className="py-24 px-6 relative z-10 bg-white/5 border-y border-white/10">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-400">How we turn complex requirements into elegant solutions.</p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="glass p-6 rounded-2xl border border-white/10 flex flex-col items-center text-center group hover:border-[#00E5FF]/50 transition-colors">
                <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-4 text-[#00E5FF] group-hover:scale-110 group-hover:bg-[#00E5FF]/10 transition-all">
                  {step.icon}
                </div>
                <h3 className="font-bold">{step.title}</h3>
                <div className="mt-2 text-sm text-gray-500 font-medium">Step 0{i + 1}</div>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={i} className="text-center p-8 glass rounded-3xl border border-white/5 hover:border-[#00E5FF]/30 transition-colors">
                <div className="text-4xl md:text-5xl font-space-grotesk font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#00E5FF] to-[#2563EB] mb-4">
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="glass p-8 rounded-3xl border border-white/10 hover:bg-white/10 transition-colors">
                <div className="text-[#00E5FF] mb-6">{feat.icon}</div>
      ...
      <section className="py-40 px-6 relative z-10 text-center">
        <div className="container mx-auto max-w-5xl">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="glass p-16 rounded-[4rem] border border-[#2563EB]/30 relative overflow-hidden shadow-[0_0_100px_-20px_rgba(37,99,235,0.2)]">
            <div className="absolute inset-0 bg-gradient-to-br from-[#2563EB]/20 via-transparent to-[#00E5FF]/20 pointer-events-none" />
