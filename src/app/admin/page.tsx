...
export default function AdminDashboard() {
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-white font-space-grotesk">Dashboard Overview</h1>
        <p className="text-gray-400 mt-2">Welcome to the DarkCode X AI admin panel.</p>
      </div>
          ...
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex items-center gap-4 p-4 bg-white/5 rounded-xl border border-white/5">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-400">admin@darkcodex.ai • 2 hours ago</p>
              </div>
            </div>
          ))}
        </div>
...
function StatCard({ title, value, icon, trend }: { title: string, value: string, icon: React.ReactNode, trend: string }) {
  return (
    <div className="glass p-6 rounded-2xl relative overflow-hidden group">
      ...
      <h3 className="text-3xl font-bold text-white mb-4 font-space-grotesk">{value}</h3>
